...
# Import correct datasets - change file_path to your own location
if dataset == 'Background':
    file_path = f"{dataset}_data.csv" 
elif dataset == 'Signal':
    file_path = f"4tops_data.csv"
...
def bin_all_background_data(list_of_dfs: list[pd.DataFrame], n_bins: int, eps:float) -> pd.DataFrame:
    ...
    df_all_data_binned[:,:,2] = devide_into_bins(df_E, n_bins, eps)[1] #E
    df_all_data_binned[:,:,1] = devide_into_bins(df_pt, n_bins, eps)[1] #pt
    df_all_data_binned[:,:,3] = devide_into_bins(df_eta, n_bins, eps)[1] #eta
...
# Store dataset of tokenized vectors as csv
df_vector_binned.to_csv(f'{dataset}_{n_bins}_.csv', index=False)
